...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    // Aseguramos capturar generarImagen desde el frontend
    const { titulo, calidad, generarImagen } = body;
    ...
    // --- 1. MOTOR DE TEXTO ---
    if (calidad === 'premium') {
      API_KEY = import.meta.env.GEMINI_API_KEY_PREMIUM || process.env.GEMINI_API_KEY_PREMIUM;
      if (!API_KEY) return new Response(JSON.stringify({ error: 'Falta GEMINI_API_KEY_PREMIUM' }), { status: 500, headers });
      urlTexto = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key=${API_KEY}`;
    } else {
      API_KEY = import.meta.env.GEMINI_API_KEY_FREE || process.env.GEMINI_API_KEY_FREE;
      if (!API_KEY) return new Response(JSON.stringify({ error: 'Falta GEMINI_API_KEY_FREE' }), { status: 500, headers });
      // Corrección: quitamos el "-latest" para asegurar estabilidad
      urlTexto = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${API_KEY}`;
    }
    ...
    if (generarImagen) {
      try {
        // La imagen siempre usa la llave Premium para evitar bloqueos
        const PREMIUM_KEY = import.meta.env.GEMINI_API_KEY_PREMIUM || process.env.GEMINI_API_KEY_PREMIUM;
        const urlImagen = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-image:predict?key=${PREMIUM_KEY}`;
        ...
        // Si Google devuelve la imagen en base64, la capturamos
        if (apiResponseImagen.ok && dataImagen.predictions && dataImagen.predictions.length > 0) {
          ...
          console.warn('Generación de imagen falló o no devolvió datos. El texto sí se guardará.', dataImagen);
        }
      } catch (errorImg) {
        console.warn('Fallo silente en la imagen:', errorImg);
      }
    }
    ...
    console.error('❌ Error general en el motor:', error.message);
