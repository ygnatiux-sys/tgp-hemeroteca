...
export function GeneradorTGP() {
  ...
  const [usarPremium, setUsarPremium] = useState(false);
  ...
  const handleGenerar = async () => {
    ...
    const calidad = usarPremium ? 'premium' : 'estandar';

    try {
      const response = await fetch('/api/generar-tgp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ titulo: tema, calidad, generarImagen }),
      });
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
        ...
        <span style={{ fontSize: '0.7rem', color: '#666' }}>v2.5 Hybrid</span>
      </div>
      ...
      >
        {loading ? 'Pensando...' : '🚀 Iniciar Generación TGP'}
      </button>
      ...
      {resultado && (
        <div style={{ marginTop: '20px', padding: '15px', background: '#111', borderRadius: '6px', border: '1px solid #222' }}>
          <p style={{ fontSize: '0.8rem', color: '#4caf50', marginBottom: '8px' }}>✓ Generación completada con éxito</p>
